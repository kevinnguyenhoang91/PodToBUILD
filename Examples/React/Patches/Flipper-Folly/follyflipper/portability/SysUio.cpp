...
#include <follyflipper/portability/SysUio.h>

#include <cerrno>
#include <cstdio>

#include <follyflipper/ScopeGuard.h>
#include <follyflipper/portability/Sockets.h>
#include <follyflipper/portability/SysFile.h>
#include <follyflipper/portability/Unistd.h>
...
static ssize_t doVecOperation(int fd, const iovec* iov, int count) {
  ...
  if (count < 0 || count > follyflipper::kIovMax) {
    errno = EINVAL;
    return -1;
  }
  ...
  bool shouldLock = !follyflipper::portability::sockets::is_fh_socket(fd);
